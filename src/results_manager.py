...
def save_results(results, category, model):
    ...
    file_path = "%s/%s_%s_results_%s.json" % (RESULTS_DIR, category, model, str(datetime.today().strftime('%Y-%m-%d_%H-%M')))
...
def save_results_raw(resultsRaw, category, model):
    file_path = "%s/%s_%s_raw-results_%s.json" % (RESULTS_DIR, category, model, str(datetime.today().strftime('%Y-%m-%d_%H-%M')))
